  ...
  context "when #call" do
    context "sending loaded product" do
      ...
      context "with invalid :product params" do
        let(:product_params) { attributes_for(:product, name: "") } #erro nome vazio
  
        it "raises NotSavedProductError" do
          expect {
            service = described_class.new(product_params, product)
            service.call
          }.to raise_error(Admin::ProductSavingService::NotSavedProductError) #levantou esse erro
        end

        it "sets validation :errors" do
          service = error_proof_call(product_params, product) #chama o método para q msm com erro ele não quebre
          expect(service.errors).to have_key(:name) #foi add ao atributo errors a chave :name
        end
      ...
      context "with invalid :productable params" do
        let(:game_params) { { productable_attributes: attributes_for(:game, developer: "") } } #erro developer vazio
...
def error_proof_call(*params) #para o erro não atrapalhar o fluxo do nosso teste
  service = described_class.new(*params)
  begin
    service.call
  rescue => e #garante que o erro não quebre o funcionamento dos testes
  end
  return service
end