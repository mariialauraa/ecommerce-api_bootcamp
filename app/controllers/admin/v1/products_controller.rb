module Admin::V1
  class ProductsController < ApiController
    ...
    def update 
      run_service
    rescue Admin::ProductSavingService::NotSavedProductError #recupera
      render_error(fields: @saving_service.errors) #devolve
    end
    ...
    def product_params #para filtrar os parâmetros para criar os produtos
    ...
    def productable_params #definir os parametros que podem vim
      productable_type = params[:product][:productable] || @product&.productable_type&.underscore #& só executa se productable e type existir
      return unless productable_type.present?
      productable_attributes = send("#{productable_type}_params") #o send monta uma chamada de um comando
      { productable_attributes: productable_attributes }
    end

    def game_params #o send chama esse método
